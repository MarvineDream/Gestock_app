...
export const createSupplier = async (req, res) => {
    ...
    try {
        const newSupplier = new fournisseur({ name });
...
export const getSupplierById = async (req, res) => {
    ...
    try {
        const supplier = await fournisseur.findById(id);
        if (!supplier) {
...
export const updateSupplier = async (req, res) => {
    ...
    try {
        const updatedSupplier = await fournisseur.findByIdAndUpdate(id, { name }, { new: true, runValidators: true });
        if (!updatedSupplier) {
            return res.status(404).json({ error: 'Fournisseur non trouvé' });
        }
...
export const deleteSupplier = async (req, res) => {
    ...
    try {
        const deletedSupplier = await fournisseur.findByIdAndDelete(id);
        if (!deletedSupplier) {
            return res.status(404).json({ error: 'Fournisseur non trouvé' });
        }
        res.status(204).send(); 
